...
import com.example.midmorningintense.databinding.ActivityMapsBinding
...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        val mombasa= LatLng(-4.0348342, 151.0)
        mMap.addMarker(MarkerOptions().position(nairobi).title("Marker in Mombasa"))
        mMap.moveCamera(CameraUpdateFactory.newLatLng(mombasa))
        val kisumu = LatLng(-0.0749726, 34.5980818)
        mMap.addMarker(MarkerOptions().position(nairobi).title("Marker in Kisumu"))
